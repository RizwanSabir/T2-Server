...
const fs = require('fs');
const { Influencer, Auidence } = require("../../models/Authentication/BaseSchema");
...
const storage = multer.diskStorage({
  ...
  router.post('/submitData', authenticateToken,upload.single('image'), async (req, res) => {
    try {
      // Extract form data
      const { fullName, userName, gender, age } = req.body;
      console.log({...req.body});
  
      const updatedBrand = await Auidence.findOneAndUpdate(
...
router.post('/check-AuidenceName', async (req, res) => {

  const {userName}=req.body
    try {
      // Check if the brand name exists in the database
      const user = await Auidence.findOne({ userName });
